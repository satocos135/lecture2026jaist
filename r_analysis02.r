...
# 自分の環境に合わせて設定
setwd('./GitHub/lecture2026shimane/')
...
## 辞書のパスを指定
# dict = 'dict/kokoro_sjis.dic' # shift-jisの辞書 Windows(R 4.2.0未満用)
# dict = '' # 辞書なし
dict = 'dict/kokoro_utf8.dic' # utf-8の辞書 Win(R 4.2.0以上用)/Mac/Linux用
...
freq_noun[!names(freq_noun) %in% stopwords] %>% sort() %>% tail(30) %>% barplot(horiz=T, las=1, main='Top 30 nouns', xlab='Frequency', cex.names=1)
...
### バイグラムを用いた共起関係の分析
bigram = docDF(parts, col='text', type=1, pos=c('名詞'), N=2, nDF=1, dic=dict) # N=2でバイグラムを指定I
dim(bigram)
